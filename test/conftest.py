...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--ipa-admin-passwd", action="store", default=environ["IPA_ADMIN_PASSWD"],
...
def pytest_generate_tests(metafunc):
    ...
    if 'ipa_admin_passwd' in metafunc.fixturenames and ipa_admin_passwd is not None:
        metafunc.parametrize("ipa_admin_passwd", [ipa_admin_passwd])
    if 'ipa_root_passwd' in metafunc.fixturenames and ipa_root_passwd is not None:
